...
void save_ppm(const std::vector<std::vector<std::array<uint8_t, 3>>> &img, const std::string &filename) {
...
struct Ray {
    ...
    std::array<uint8_t, 3> backward(const glm::vec3 &light, const Triangle &t) {
        ...
        if (angle_light_triangle > PI / 2) {
            angle_light_triangle = PI - angle_light_triangle;
        }
...
struct Camera {
    ...
    Camera(const glm::vec3 &eye, const float &d, const float &w, const float &h, const float &delta)
        : eye(eye), distance_to_screen(d), width(w), height(h), delta(delta) {}
