...
HierarchySidebar::HierarchySidebar(float maxWidth_, float slideSpeed_)
    : sidebarOpen(false), sidebarWidth(0.0f), maxWidth(maxWidth_), slideSpeed(slideSpeed_), lightsOpen(true), objectsOpen(true),
...
void HierarchySidebar::draw(int windowWidth, int windowHeight) {
    ...
    if (sidebarWidth > 0.0f) {
        ...
        ImGui::SetNextWindowPos(ImVec2(static_cast<float>(windowWidth) - static_cast<float>(sidebarWidth), menuBarHeight));
        ...
        if (ImGui::CollapsingHeader("Objects", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            for (int i = 0; i < objectNames.size(); i++) {
                ...
                bool isSelected = (selectedType == SelectedType::Object && selectedIndex == i);
                ...
                if (renamingType == SelectedType::Object && renamingIndex == i) {
                    // Renaming mode input
                    if (ImGui::InputText("##rename", renameBuffer.data(), renameBuffer.size(), ImGuiInputTextFlags_EnterReturnsTrue)) {
                    ...
                    if (!isRenaming) {
                        if (ImGui::Selectable(label.c_str(), isSelected, 0, ImVec2(textWidth, 0))) {
                            selectedIndex = i;
                            selectedType = SelectedType::Object;
                        }
                    } else {
                        // Disabled selectable look
                        ImGui::Selectable(label.c_str(), isSelected, ImGuiSelectableFlags_Disabled, ImVec2(textWidth, 0));
                    }

                    if (isRenaming)
                        ImGui::PopStyleVar();

                    if (!isRenaming && ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0)) {
                        renamingIndex = i;
                ...
                if (ImGui::Button(eyeText, ImVec2(25, 0))) {
                    objectVisible[i] = !objectVisible[i]; // TODO: toggle object visibility
                }
        ...
        if (ImGui::CollapsingHeader("Lights", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            for (int i = 0; i < lightNames.size(); i++) {
                ...
                bool isSelected = (selectedType == SelectedType::Light && selectedIndex == i);
                ...
                // --- Buttons ---
                float buttonX = ImGui::GetWindowContentRegionMax().x - 37.0f; // Align with object bin button
                ImGui::SameLine(buttonX);
    ...
    float menuBarHeight = ImGui::GetFrameHeight();
    ImVec2 buttonPos(static_cast<float>(windowWidth) - sidebarWidth - buttonSize, menuBarHeight);
    ...
    if (mouse.x >= buttonPos.x && mouse.x <= buttonPos.x + buttonSize && mouse.y >= buttonPos.y && mouse.y <= buttonPos.y + buttonSize &&
    ...
    if (sidebarOpen) {
        ...
        draw_list->AddTriangleFilled(ImVec2(center.x - halfSize, center.y - halfSize), ImVec2(center.x - halfSize, center.y + halfSize),
