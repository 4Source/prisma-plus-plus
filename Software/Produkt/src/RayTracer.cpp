...
RayTracer::RayTracer(const Scene& s) : scene(s){
...
void RayTracer::start(){
	...
	for(int i = 0; i < pixels_v; i++) {
    	view.push_back(std::vector<glm::vec3>{});
       	for(int j = 0; j < pixels_h; j++) {
            view.at(i).push_back(glm::vec3{255, 0, 255});
        }
    }
	...
	for(int i = 0; i < pixels_v; i++) {
        for(int j = 0; j < pixels_h; j++) {
			glm::vec3 point = top_left + i * scene.camera.resolution * left_norm + j * scene.camera.resolution * top_norm;
...
std::vector<std::vector<std::array<uint8_t, 3>>> RayTracer::view_to_rgb(){
	...
	for(int i = 0; i < view.size(); i++){
		rgb.push_back(std::vector<std::array<uint8_t, 3>>{});
		for(int j = 0; j < view.at(i).size(); j++){
			...
			rgb.at(i).push_back(std::array<uint8_t, 3>{r, g, b});
		}
	}
	return rgb;
}
